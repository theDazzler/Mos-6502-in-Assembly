...
// To call from SPARC assembly language:
// Inputs:
// %o0 - Memory address to get data from. Address should be a 16-bit SPARC halfword.
//       Higher order bits are ignored.
//
// Outputs:
// %o0 - The value stored at the given address. This will be a byte, stored in the rightmost
//       8 bits of %o0. The leftmost 24 bits will be 0.
...
// To call from SPARC assembly language:
// Inputs:
// %o0 - Memory address to store data at. Address should be a 16-bit SPARC halfword.
//       Higher order bits are ignored.
//
// %o1 - Value to store at the address given in %o0. This should be a byte, stored in
//       the rightmost 8 bits of %o1. The leftmost 24 bits should be 0.
// Outputs:
// Nothing
...
void indirect6502()
{
      help = memImage[PC] + (memImage[PC + 1] << 8);
      savepc = memImage[help] + (memImage[help + 1] << 8);
      PC++;
      PC++;
}
...
void absx6502()
{
      savepc = memImage[PC] + (memImage[PC + 1] << 8);
      PC++;
      PC++;
      if (ticks[opcode]==4)
...
{
      value = memImage[PC++]+X;
      savepc = memImage[value] + (memImage[value+1] << 8);
}
...
void indabsx6502()
{
      help = memImage[PC] + (memImage[PC + 1] << 8) + X;
      savepc = memImage[help] + (memImage[help + 1] << 8);
}
...
void brk6502()
{
      ...
      P |= 0x14;
      PC = memImage[0xfffe & addrmask] + (memImage[0xffff & addrmask] << 8);
}
...
void tsb6502()
{
      adrmode[opcode]();
      memImage[savepc] |= A;
      if (memImage[savepc]) P &= 0xfd; else P |= 0x02;
}

void trb6502()
{
      adrmode[opcode]();
      memImage[savepc] = memImage[savepc] & (A ^ 0xff);
      if (memImage[savepc]) P &= 0xfd; else P |= 0x02;
}
...
// Super-budget fake memory-mapped I/O. It has limited capacities.
// All it can do is dial and yell. I mean: read and write characters to
// stdout.
//
// Details:
// $EF00 - I/O status register
//			bit 0: execute write
//			bit 1: execute read
// $EF01 - output character
// $EF02 - input character
void fakeIO()
{
	// If write bit is set, write from output register and
	// clear write bit.
	if (memImage[0xEF00] & 1) {
		printf("%c\n", memImage[0xEF01]);
		//fflush(stdout);
		memImage[0xEF00] &= ~1;
	}
	
	// If read bit is set, read a charater, put it in the
	// input register and clear the read bit.
	if (memImage[0xEF00] & (1 << 1)) {
		memImage[0xEF02] = getchar();
		memImage[0xEF00] &= ~(1<<1);
	}
